...
namespace FollowBot.Tasks
{
    class TravelToPartyZoneTask : ITask
    {
        ...
        public async Task<bool> Run()
        {
            ...
            if (campaignLabPortal != null)
            {
                Log.DebugFormat("[{0}] Found campaign lab portal.", Name);
                if (LokiPoe.Me.Position.Distance(campaignLabPortal.Position) > 15)
                {
                    ...
                    Move.Towards(walkablePosition, "moving to campaign lab portal");
                    return true;
                }
                ...
                if (!tele)
                {
                    Log.DebugFormat("[{0}] campaign lab portal error.", Name);
                }
